CREATE OR REPLACE PACKAGE BODY RENTAL AS
    ...
    BEGIN
        TOTAL_ID_ARRAY := ID_ARRAY();
        FOR I IN 1..T_DELIVERY_DICTIONARY.LAST LOOP
    ...
    BEGIN
        ...
        WHILE BOOKS_LEFT > 0 LOOP
            ...
            IF FREE_SPOTS_IN_LOCATION > 0 THEN
                ...
                REQUEST_LOCK_RET_CODE := LOCK_LOCATION(LOCATION_ID_ARRAY(I));
                
                CASE REQUEST_LOCK_RET_CODE
                
                    WHEN 0 THEN -- BLOKADA UDANA
                        DBMS_OUTPUT.PUT_LINE('LOCATION ' || TO_CHAR(LOCATION_ID_ARRAY(I)) || ' HAS JUST BEEN LOCKED');
                    ...
                    WHEN 2 THEN -- DEADLOCK
                        DBMS_OUTPUT.PUT_LINE('DEADLOCK TRYING TO LOCK LOCATION ' || TO_CHAR(LOCATION_ID_ARRAY(I)));
                    WHEN 3 THEN -- PARAMETER ERROR
                        DBMS_OUTPUT.PUT_LINE('PARAMETER ERROR TRYING TO LOCK LOCATION ' || TO_CHAR(LOCATION_ID_ARRAY(I)));
                    ...
                    WHEN 5 THEN -- ILLEGAL LOCKHANDLE
                        DBMS_OUTPUT.PUT_LINE('ILLEGAL LOCKHANDLE TRYING TO LOCK LOCATION ' || TO_CHAR(LOCATION_ID_ARRAY(I)));
                END CASE;
    ...
    BEGIN
        WHILE BOOKS_LEFT > 0 LOOP
            ...
            CASE RELEASE_LOCK_LOCATION_RET_CODE
                ...
                WHEN 3 THEN
                    DBMS_OUTPUT.PUT_LINE('PARAMETER ERROR RELEASING LOCK ON LOCATION_ID ' || TO_CHAR(PREV_LOCATION_ID));
                WHEN 4 THEN
                    DBMS_OUTPUT.PUT_LINE('WARNING: LOCK ON LOCATION_ID ' || TO_CHAR(PREV_LOCATION_ID)
                        || ' ALREADY RELEASED');
                WHEN 5 THEN
                    DBMS_OUTPUT.PUT_LINE('ILLEGAL LOCKHANDLE RELEASING LOCK ON LOCATION_ID ' || TO_CHAR(PREV_LOCATION_ID));
            END CASE;
            DBMS_OUTPUT.PUT_LINE('');
    ...
    EXCEPTION
        WHEN WRONG_LOCATION_ID THEN
            DBMS_OUTPUT.PUT_LINE('LOCK_LOCATION() ERROR: LOCATION_ID ' || TO_CHAR(T_LOCATION_ID)
                || ' IS INCORRECT');
    END LOCK_LOCATION;
    ...
    EXCEPTION
        WHEN WRONG_LOCATION_ID THEN
            DBMS_OUTPUT.PUT_LINE('RELEASE_LOCK_LOCATION() ERROR: LOCATION_ID ' || TO_CHAR(T_LOCATION_ID)
                || ' IS INCORRECT');
